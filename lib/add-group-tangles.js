...
module.exports = function AddGroupTangles(server) {
  const getTangle = {
    group: GetGroupTangle(server, 'group'),
    member: GetGroupTangle(server, 'member'),
  }

  function addTangles(content, tangles, cb) {
    ...
    getTangle[currTangle](content.recps[0], (err, generatedTangle) => {
      ...
      return addTangles(content, tangles.slice(1), cb)
    })
  }

  /**
   * Note that this mutates `content`
   * `tangles` is an array like ["group", "member"]
   */
  return function addGroupTangles(content, tangles, cb) {
