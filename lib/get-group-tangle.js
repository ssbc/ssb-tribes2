...
/** `server` is the ssb server you're using. `tangle` is the name of the tangle in the group you're looking for, e.g. "group" or "member" */
module.exports = function GetGroupTangle(server, tangle) {
  const getUpdates = GetUpdates(server, tangle)

  return function getTangle(groupId, cb) {
    if (!isIdentityGroupSSBURI(groupId)) {
      // prettier-ignore
      return cb(new Error(`get-group-tangle expects valid groupId, got: ${groupId}`))
    }

    server.box2.getGroupKeyInfo(groupId, (err, info) => {
      // prettier-ignore
      if (err) return cb(clarify(err, 'Failed to get group key info when getting a group tangle'))
      ...
      getUpdates(info.root, (err, msgs) => {
        // prettier-ignore
        if (err) return cb(clarify(err, 'Failed to read updates when getting group tangle'))
