...
module.exports = function AddGroupTangle(server) {
  ...
  const getMemberTangle = GetGroupTangle(server, 'member')
  ...
  return function addGroupTangle(content, cb) {
    ...
    getGroupTangle(content.recps[0], (err, tangle) => {
      ...
      getMemberTangle(content.recps[0], (err, tangle) => {
        // prettier-ignore
        if (err) return cb(clarify(err, 'Failed to get member tangle when adding member tangle to message'))

        set(content, 'tangles.member', tangle)

        content.tangles.member.previous = content.tangles.member.previous.sort(
          () => (Math.random() < 0.5 ? -1 : +1)
        )
