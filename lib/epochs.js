...
module.exports = function Epochs(ssb) {
  ...
  function getEpochs(groupId, cb) {
    ...
    reduceEpochs(
      ssb,
      groupId,
      { getters: pluck(allGetters, ['author', 'secret']) },
      (err, reduce) => {
        if (err)
  ...
  function getMembers(epochRootId, cb) {
    ...
    pull(
      getMembers.stream(epochRootId),
      pull.drain(
        (update) => {
          if (update.added) update.added.forEach((feedId) => added.add(feedId))
          else update.excluded.forEach((feedId) => toExclude.add(feedId))
        },
        (err) => {
          // prettier-ignore
  ...
  getMembers.stream = function getMembersStream(epochRootId, opts = {}) {
    return pull(
      ...
      pull.map((msg) => {
        ...
        if (type === 'group/add-member') return { added: recps.slice(1) }
        else return { excluded: excludes }
      })
    )
  }
  ...
  getPreferredEpoch.stream = function getPreferredEpochStream(
    groupId,
    opts = {}
  ) {
    const { live } = opts
...
function reduceEpochs(ssb, groupId, opts = {}, cb) {
  ...
  pull(
    getEpochNodeStream(ssb, groupId, { getters }),
...
reduceEpochs.stream = function reduceEpochsStream(ssb, groupId, opts = {}) {
  ...
  return pull(
    getEpochNodeStream(ssb, groupId, { getters, live }),
    pull.through((node) => reduce.addNodes([node])),
    pull.map((node) => {
      reduce.resolve()
      return reduce
    })
...
function getEpochNodeStream(ssb, groupId, opts = {}) {
  const { getters, live } = opts
  const deferedSource = pullDefer.source()

  ssb.box2.getGroupInfo(groupId, (err, info) => {
    // prettier-ignore
    if (err) return deferedSource.abort(clarify(err, 'Failed to get group info for ' + groupId))

    // Fetch the tangle root
    ssb.db.get(info.root, (err, rootVal) => {
      // prettier-ignore
      if (err) return deferedSource.abort(clarify(err, 'Failed to load group root with id ' + info.root))

      if (!isInitRoot(rootVal))
        // prettier-ignore
        return deferedSource.abort(clarify(new Error(isInitRoot.string), 'Malformed group/init root message'))
      ...
      const source = pull(
        pull.values([
          pull.values([root]), // TODO pull.once
          pull(
            getTangleUpdates.stream(ssb, 'epoch', info.root, { live }),
          ...
          pull(
            ...
            pull.asyncMap(([fieldName, getter], cb) => {
              getter(msg, (err, fieldData) => {
                // prettier-ignore
                if (err) return deferedSource.abort(clarify(err, 'Failed to get epoch ' + fieldName))
            ...
            pull.collect((err) => {
              // prettier-ignore
              if (err) return deferedSource.abort(clarify(err, 'Failed to collect epoch data'))
