...
module.exports = function Epochs(ssb) {
  ...
  function getMembers(epochRootId, cb) {
    ...
    pull(
      ...
      pull.drain(
        (update) => {
          update.added.forEach((feedId) => added.add(feedId))
          update.toExclude.forEach((feedId) => toExclude.add(feedId))
        },
        (err) => {
          // prettier-ignore
    ...
    return pull(
      ...
      pull.map((msg) => {
        ...
        if (type === 'group/add-member')
          return { added: recps.slice(1), toExclude: [] }
        else return { added: [], toExclude: excludes }
      })
    )
  }
