  ...
  return {
    ...
    getPickedEpoch(groupId, opts) {
      ...
      return pull(
        ...
        pull.map((groupInfo) => {
          // prettier-ignore
          if (groupInfo.excluded) throw new Error("Can't get picked epoch for group we're excluded from")

          return groupInfo
        }),
        // only pass forward group info if the writeKey is different to last time
        pull.filter((groupInfo) => {
          if (lastKey?.equals(groupInfo.writeKey.key)) return false

          lastKey = groupInfo.writeKey.key
          if (!lastKey) console.log('empty groupinfo?', groupInfo)
          return true
        }),
        // find details about the newly picked epoch
        pull.asyncMap((groupInfo, cb) => {
          getEpochs(groupId, (err, epochs) => {
            // prettier-ignore
            if (err) return cb(clarify(err, "Error getting info about epochs when looking for picked epoch"))

            console.log('got epochs but no err', epochs, 'groupInfo', groupInfo)

            cb(
              null,
              epochs.find((epoch) =>
                epoch.secret.equals(groupInfo.writeKey.key)
              )
            )
          })
        })
    ...
    getMemberUpdates(epochRootId, opts) {
      return getMemberUpdates(ssb, epochRootId, opts)
    },
    ...
    getMembers(epochRootId, cb) {
      if (cb === undefined) return p(this.getMembers).call(this, epochRootId)
...
function getMemberUpdates(ssb, epochRootId, opts) {
  return pull(
    ...
    pull.map((msg) => {
      ...
      if (type === 'group/add-member')
        return recps.slice(1).map((feedId) => ({ added: feedId }))
      else return excludes.map((feedId) => ({ excluded: feedId }))
    }),
    pull.flatten(),
    // grab updates until we get an exclusion, then close the stream (even if live)
    // last: true includes the exclusion as well right before closing
    pull.take((update) => update.added, { last: true })
  )
}
...
function getMembers(ssb, epochRootId, cb) {
  ...
  pull(
    getMemberUpdates(ssb, epochRootId),
    pull.drain(
      (update) => {
        if (update.added) added.add(update.added)
        else toExclude.add(update.excluded)
      },
      (err) => {
        // prettier-ignore
