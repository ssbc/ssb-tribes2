...
module.exports = function Epochs(ssb) {
  ...
  return {
     ...
     */
    getPredecessorEpochs(groupId, epochRootId, cb) {
      if (cb === undefined)
        return p(this.getPredecessorEpochs).call(this, groupId, epochRootId)

      getEpochs(groupId, (err, epochs) => {
        // prettier-ignore
        if (err) return cb(clarify(err, "Couldn't get epochs when getting predecessors"))

        const predecessorIds = new Set()
        predecessorIds.add(epochRootId)

        // bfs down into the `previous` epochs
        predecessorIds.forEach((predecessorId) => {
          const epoch = epochs.find((epoch) => epoch.id === predecessorId)

          epoch.previous?.forEach((previousId) => {
            predecessorIds.add(previousId)
          })
        })

        const predecessors = [...predecessorIds]
          // remove the epoch that we pointed to
          .filter((predId) => predId !== epochRootId)
          .map((predId) => epochs.find((epoch) => epoch.id === predId))
  ...
  ssb.box2.getGroupInfo(groupId, (err, info) => {
    ...
    ssb.db.get(info.root, (err, rootVal) => {
      ...
      if (!isInitRoot(rootVal))
        // prettier-ignore
        return cb(new Error('Malformed group/init root message: ' + isInitRoot.errorsString))
