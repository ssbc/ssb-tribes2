  ...
  return {
    ...
    getPickedEpoch(groupId, opts) {
      ...
      return pull(
        ...
        pull.map((groupInfo) => {
          // prettier-ignore
          console.log('groupinfo update raw', groupInfo)
          if (groupInfo.excluded)
            throw new Error(
              "Can't get picked epoch for group we're excluded from"
            )
        ...
        pull.filter((groupInfo) => {
          ...
          if (!groupInfo.writeKey) console.log('missing key?', groupInfo)
