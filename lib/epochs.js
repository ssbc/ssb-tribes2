    ...
    getPickedEpoch(groupId, opts) {
      ...
      return pull(
        ...
        pull.asyncMap((groupInfo, cb) => {
          // prettier-ignore
          if (groupInfo.excluded) return cb(new Error("Can't get picked epoch for group we're excluded from"))

          this.getEpochs(groupId, (err, epochs) => {
            // prettier-ignore
            ...
            cb(
              null,
              epochs.find((epoch) =>
                epoch.epochKey.equals(groupInfo.writeKey.key)
              )
            )
          })
        })
